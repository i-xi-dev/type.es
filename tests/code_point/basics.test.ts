...
Deno.test("CodePoint.fromRune()", () => {
  assertStrictEquals(CodePoint.fromRune("\u0000"), 0x0);
  assertStrictEquals(CodePoint.fromRune("\uFFFF"), 0xFFFF);
  assertStrictEquals(CodePoint.fromRune("\u{10000}"), 0x10000);
  assertStrictEquals(CodePoint.fromRune("\u{10FFFF}"), 0x10FFFF);
  ...
  assertThrows(
    () => {
      CodePoint.fromRune(0 as unknown as string);
    },
    TypeError,
    "`rune` must be an Unicode scalar value.",
  );
